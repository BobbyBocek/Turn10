...
export default function NewMatch() {
  ...
  const [posSheet, setPosSheet] = useState({ open: false, uid: null });
    ...
    Promise.all([api.get("/users"), api.get("/rules"), api.get("/positions")])
      .then(([u, r, p]) => {
        ...
        if (prefill.players) {
          ...
          setActiveRuleIds(prefill.ruleIds || []);
        }
      })
  ...
  const setPos = (uid, pos) => { setSelected((prev) => prev.map((s) => (s.user_id === uid ? { ...s, table_position: pos } : s))); setPosSheet({ open: false, uid: null }); };
  ...
  const save = async () => {
    ...
    try {
      const participants = order.map((uid, idx) => {
        ...
        const c = cards[uid];
        return { user_id: uid, placement: idx + 1, table_position: s.table_position || null, exit_card_value: c?.value ?? null, exit_card_suit: c?.suit ?? null };
      });
  ...
  const tablePlayers = selected.map((s) => ({ id: s.user_id, name: s.name, icon: s.icon, position: s.table_position }));
  ...
  return (
    <div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }}>
              <RoundTable players={tablePlayers} onAvatarClick={(uid) => setPosSheet({ open: true, uid })} />
              <p className="text-center text-xs text-slate-500 mb-3 -mt-1">Tryck på en spelare vid bordet för att tilldela bordsposition.</p>
              ...
              <div className="space-y-2">
                {filtered.map((u) => {
                  const sel = selected.find((p) => p.user_id === u.id);
                  return (
                    <button key={u.id} data-testid={`player-select-checkbox-${u.id}`} onClick={() => toggleUser(u)}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-slate-500 font-mono">{u.rating} • {u.matches_played} matcher{sel?.table_position ? ` • ${sel.table_position}` : ""}</div>
                      </div>
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }}>
              ...
              {rules.length === 0 && <p className="text-sm text-slate-500 py-6 text-center">Inga regler i biblioteket ännu.<br />Tryck "Ny regel" för att lägga till.</p>}
      ...
      <AnimatePresence>
        {posSheet.open && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] bg-black/70 flex items-end justify-center" onClick={() => setPosSheet({ open: false, uid: null })}>
            <motion.div initial={{ y: 250 }} animate={{ y: 0 }} exit={{ y: 250 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-slate-900 border-t border-amber-500/30 rounded-t-2xl p-5 pb-8">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-slate-100 font-display">Bordsposition</h3>
                <button data-testid="close-pos-sheet" onClick={() => setPosSheet({ open: false, uid: null })}><X className="w-5 h-5 text-slate-400" /></button>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <button data-testid="pos-clear" onClick={() => setPos(posSheet.uid, "")} className="py-3 rounded-xl bg-slate-800 border border-slate-700 text-slate-400 text-sm">Ingen</button>
                {positions.map((p) => (
                  <button key={p.id} data-testid={`pos-option-${p.name}`} onClick={() => setPos(posSheet.uid, p.name)}
                    className="py-3 rounded-xl bg-slate-800 border border-slate-700 text-slate-200 text-sm active:bg-amber-500/20 active:border-amber-500">{p.name}</button>
                ))}
              </div>
            </motion.div>
          </motion.div>
